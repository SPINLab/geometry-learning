SELECT st_astext(st_snaptogrid(gebouw.wkb_geometry, 0.000001)) AS brt_wkt,
  ...
  st_astext(st_snaptogrid(st_centroid(gebouw.wkb_geometry), 0.000001)) AS brt_centroid_wkt,
  st_astext(st_snaptogrid(st_centroid(osm_buildings.wkb_geometry), 0.000001)) AS osm_centroid_wkt,
  st_astext(st_snaptogrid(st_transform(st_centroid(gebouw.wkb_geometry), 28992), 0.000001)) AS brt_centroid_wkt_rd,
  st_astext(st_snaptogrid(st_transform(st_centroid(osm_buildings.wkb_geometry), 28992), 0.000001)) AS osm_centroid_wkt_rd
