...
def np_gaussian_2d_loss(true, pred):
    ...
    sigma_x = np.abs(pred[:, 2])
    sigma_y = np.abs(pred[:, 3])
    rho = pred[:, 4]  # hardcode to avoid drifting to -1 or 1

    norm1 = np.log(1 + np.abs(x_coord - mu_x))
    norm2 = np.log(1 + np.abs(y_coord - mu_y))

    variance_x = np.exp(np.square(sigma_x))
    variance_y = np.exp(np.square(sigma_y))
    s1s2 = np.exp(sigma_x * sigma_y)  # very large if sigma_x and/or sigma_y are very large
