...
brt_geoms = [GeoVectorizer().decypher(vector) for vector in brt_vectors]
osm_geoms = [GeoVectorizer().decypher(vector) for vector in osm_vectors]

geo_interfaces = [wkt.loads(brt_geoms[0]).__geo_interface__, wkt.loads(osm_geoms[0]).__geo_interface__]
